...
// Vertex Cover Algorithm using Edmond Karp's Algorithm
public class VertexCover	{
	...
	private static void compute(String filename) throws IOException{
		...
		BufferedWriter bw = new BufferedWriter(new FileWriter("output/VC-"+filename));
		...
		do {
			...
			BufferedWriter out = new BufferedWriter(new FileWriter("output/VC-"+filename+"-"+(iterationCount)));
			br = new BufferedReader(new FileReader(input));
			while((line = br.readLine()) != null)	{
				String[] nodes =  line.split("\t");
			...
			out.close();
			input = "output/VC-"+filename+"-"+(iterationCount);
			File f = new File("output/VC-"+filename+"-"+(iterationCount-1));
			if(f.exists())
				f.delete();
			iterationCount++;
