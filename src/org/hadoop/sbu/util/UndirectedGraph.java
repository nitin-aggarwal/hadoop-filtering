...
public final class UndirectedGraph<T> implements Iterable<T> {
    ...
    /**
     * Merges two undirected graphs into one.
     * @author mukul
     */
    public void mergeAll(UndirectedGraph<T> inGraph){
    	Iterator<T> itr = inGraph.iterator();
		while(itr.hasNext()){
			//get first node from inGraph
			T node = itr.next();
			this.addNode(node);
			//get edge from inGraph
			for(T nodeOut : inGraph.edgesFrom(node)){
				this.addNode(nodeOut);
				this.addEdge(node, nodeOut);
			}
		}
    ...
    public void clear()	{
		Iterator<T> iter = mGraph.keySet().iterator();
    	while(iter.hasNext() )	{
    		T obj = iter.next();
    		mGraph.get(obj).clear();
    	}
    	mGraph.clear();
    }
}